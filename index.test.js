...
describe('Empty line', () => {
    test('Apply heading to empty line', () => {
        expect(markdo('', 'heading1', {start: 0, end: 0}))
            .toBe('# ');
    });

    test('Apply bold to empty line', () => {
        expect(markdo('', 'bold', {start: 0, end: 0}))
            .toBe('****');
    });

    test('Apply italic to empty line', () => {
        expect(markdo('', 'italic', {start: 0, end: 0}))
            .toBe('__');
    });

    test('Apply quote to empty line', () => {
        expect(markdo('', 'quote', {start: 0, end: 0}))
            .toBe('> ');
    });

    test('Apply link to empty line', () => {
        expect(markdo('', 'link', {start: 0, end: 0}))
            .toBe('[](url)');
    });

    test('Apply bulleted list to empty line', () => {
        expect(markdo('', 'bulletedList', {start: 0, end: 0}))
            .toBe('- ');
    });

    test('Apply ordered list to empty line', () => {
        expect(markdo('', 'orderedList', {start: 0, end: 0}))
            .toBe('1. ');
    });

    test('Apply task list to empty line', () => {
        expect(markdo('', 'taskList', {start: 0, end: 0}))
            .toBe('- [ ] ');
    });
});
...
> events.`);
    ...
    test('Apply bulleted list to the third word, cursor after the word', () => {
        expect(markdo(line, 'bulletedList', {start: 13, end: 13}))
            .toBe(`Think of 
...
describe('Multiple lines text', () => {
    test('Apply bold to the word in the midlle of second line', () => {
        expect(markdo(lines, 'bold', {start: 40, end: 40}))
...
Executing the Observable`);
    ...
    test('Apply quote to selection of two lines of text', () => {
        expect(markdo(lines, 'quote', {start: 0, end: 47}))
            .toBe(`> Creating Observables
> Subscribing to Observables
...
Executing the Observable`);
    ...
    test('Apply bulleted list to selection of two lines of text', () => {
        expect(markdo(lines, 'bulletedList', {start: 0, end: 47}))
            .toBe(`- Creating Observables
- Subscribing to Observables
...
Executing the Observable`);
    ...
    test('Apply ordered list to selection of two lines of text', () => {
        expect(markdo(lines, 'orderedList', {start: 0, end: 47}))
...
Executing the Observable`);
    ...
    test('Apply bold to new line of multiline text', () => {
        expect(markdo(linesWithEmpty, 'bold', {start: 11, end: 11}))
            .toBe(`Schedulers
****`);
    });

    test('Apply quote to new line of multiline text', () => {
        expect(markdo(linesWithEmpty, 'quote', {start: 11, end: 11}))
            .toBe(`Schedulers
...
> `);
    ...
    test('Apply bulleted list to new line of multiline text', () => {
        expect(markdo(linesWithEmpty, 'bulletedList', {start: 11, end: 11}))
            .toBe(`Schedulers
...
- `);
    ...
    test('Apply ordered list to new line of multiline text', () => {
        expect(markdo(linesWithEmpty, 'orderedList', {start: 11, end: 11}))
            .toBe(`Schedulers
...
test('Apply unknown style', () => {
    expect(markdo(line, 'avada-kedavra', {start: 0, end: 5}))
        .toBe('Think of RxJS as Lodash for events.');
});